...
(** I need a RegSet ordered by degree. So to keep tack of the registers in the graph and easily checking if there is 
  some register with degree less than K.
Then I need a RegSet ordered by some heuristic condition, every time I extract a node from this 
set i need to check if it is in the graph(meaning it is in the set above) if it is not then just throw and take again the maximum.
Then I need a RegMap for each register which points to a RegSet ordered by colors, 
an uncolored color has -INF value and i need to retrieve the maximum *)
...
(* The Chaitin-Briggs coloring algorithm which colors an interference graph with these steps:
...
*)
val chaitin_briggs_algorithm: mriscfg -> degree -> interference_graph
