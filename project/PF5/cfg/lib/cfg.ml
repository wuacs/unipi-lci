...
Each code's block is traversed from beginning to end.
...
let convert_minirisc_bop (register : Minirisc.register) (input : 'a * 'a)
  ...
  | x, y when Option.is_some (coerce x) ->
      ...
      (first eval_right @ ir t right_register register, second eval_right, third eval_right)
  | x, y when Option.is_some (coerce y) ->
      ...
      (first eval_left @ ri left_register t register, second eval_left, third eval_left)
  | x, y ->
      let left_register = get_new_register register in
      let eval_left = helper_fun left_register x id_ram in
      let right_register = get_new_register (second eval_left) in
      let eval_right = helper_fun right_register y (third eval_left) in
      ( first eval_left @ first eval_right @ rr left_register right_register register,
...
let convert_miniimp_arithmetic_to_minirisc (available : Minirisc.register)
  ...
  let rec helper_arithemtic (register : Minirisc.register) (expr : coercable)
    ...
    | Integer expr -> (
        match expr with
        | ImpAst.Minus (x, y) ->
            convert_minirisc_bop register (Integer x, Integer y) id_ram
              helper_arithemtic
        ...
        | ImpAst.Substitue s ->
            let id_ram = reserve_opt_variable_register id_ram s register in
            ( [Minirisc.Rury (Minirisc.Copy, StringMap.find s id_ram, register)],
              register, id_ram)
        | ImpAst.Aval n ->
            ( [
              Minirisc.LoadI (n, register)
            ],
            register, id_ram))
...
let convert_miniimp_boolean_to_minirisc (available : Minirisc.register)
  ...
  let rec helper_boolean (register : Minirisc.register) (expr : coercable)
    ...
    | Boolean expr -> (
        match expr with
        | ImpAst.And (x, y) ->
            convert_minirisc_bop register (Boolean x, Boolean y) id_ram
              helper_boolean
        ...
        | ImpAst.Minor (x, y) ->
            let left_register = get_new_register register in
            let left_expr =
              convert_miniimp_arithmetic_to_minirisc left_register x id_ram
            in
            let right_register = get_new_register (second left_expr) in
            let right_expr =
              convert_miniimp_arithmetic_to_minirisc right_register y (third left_expr)
            in
            ( first left_expr @ first right_expr
              ...
              second right_expr, third right_expr)
        | ImpAst.Not x -> 
          let eval_reg = get_new_register available in
          let eval_expr = helper_boolean eval_reg (Boolean x) id_ram in
          ( first eval_expr @ [Minirisc.Rury(Minirisc.Not, eval_reg, available)],
          second eval_expr, third eval_expr)
        | _ -> failwith "Only Add, Less and Not of miniimp's boolean instructions supported")
...
let miniimp_cfg_to_minirisc (imp_cfg : ImpAst.miniimp_simple control_flow_graph)
  ...
  let map_simple_imp_to_simple_risc (stmt : ImpAst.miniimp_simple)
    ...
    | ImpAst.Assignment (str, aexp) ->
        let string_to_ram = reserve_opt_variable_register string_to_ram str available in
           ...
           let res =
             List.fold_left
               (fun (accumulated_stmts, current_register, ram) stmt ->
                 ...
                 in
                 (accumulated_stmts @ first blk_res, (get_new_register (second blk_res)), third blk_res))
               ([], available, ram) stmts
           in
           (NodeMap.add nodeId (first res) map, get_new_register (second res), third res))
