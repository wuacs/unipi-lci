...
MiniImp program translated to MiniRISC where each node contains the set of registers
...
let () =
  ...
  | "undefinedness" -> if Array.length Sys.argv <> 4 then
      fail_helper ~additional_info:"`undefinedness takes 2 parameters" () else
      compute_defined_analysis_and_create_dot ()
  | "liveness" ->  if Array.length Sys.argv <> 4 then
      fail_helper ~additional_info:"`liveness takes 2 parameters`" () else
      compute_live_analysis_and_create_dot ()
  | _ -> fail_helper ()