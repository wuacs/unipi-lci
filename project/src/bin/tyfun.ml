...
To interpret a MiniTyFun program written somewhere in a file and output its result\
to stdout use:
...
let () = 
  ...
  if num_arguments < 2 then fail_helper ("You need to specify which functionality to use according as suggested below") else
  let choice = Sys.argv.(1) in
  match choice with
  | "interpret_file" -> if num_arguments <> 3 then fail_helper ("You need to specify an input file") else execute_miniimp_from_file ()
