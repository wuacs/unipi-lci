...
let find_node_safe (node : node) (map : 'a Nodemap.t) (description : string) :
    'a =
  try Nodemap.find node map
  with Not_found ->
    raise
      (NodeNotFound
         (Printf.sprintf "Node not found in %s: %d" description
            (match node with Cfg.Label x -> x)))
...
let update_in (cfg : mriscfg) (node : node) (stateMap : stateMap) : stateMap =
  try
    ...
    let new_in =
      RegSet.union
        (Data_flow_utils.used_registers cfg node)
...
let coalesced_edge_map (cfg : mriscfg) : node list Nodemap.t =
  try
    Nodemap.add cfg.exit []
      (Nodemap.fold
         (fun node next coalesced ->
           Cfg.NodeMap.add node
             (match next with
             | Cfg.Uncond t -> [ t ]
             | Cfg.Cond (t, t1) -> [ t; t1 ]
...
let liveness_analysis (cfg : mriscfg) : stateMap =
  try
    ...
    let rec compute_fixpoint (cfg : mriscfg) (equal : bool) (state : stateMap) :
        stateMap =
      try
        match equal with
        | true -> state
