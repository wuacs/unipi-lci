...
let init () =
  let rec 
  guard_left = { l1 = guard_left; l2 = guard_right; v = None } 
  and guard_right = { l1 = guard_left; l2 = guard_right; v = None }
  in {front = guard_left; back = guard_right }
...
let push_front x q = 
  put_between q.front q.front.l2 x

let push_back x q =
  put_between q.back q.back.l1 x

let is_back b q = 
  if b.l2 == q.back then true else false

let is_front t q = 
  if t.l1 == q.front then true else false
