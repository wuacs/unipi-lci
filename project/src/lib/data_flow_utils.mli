...
val used_registers :
  Minirisc.scomm Cfg.control_flow_graph -> Cfg.node -> Minirisc.RegisterSet.t
(** Extracts the used registers in the node. A used register is such a register
    which is read before being written. *)

val get_top : Minirisc.scomm Cfg.control_flow_graph -> Minirisc.RegisterSet.t
(** Gets all the registers defined in the CFG *)

val extract_written_register : Minirisc.scomm -> Minirisc.register option
val extract_read_registers : Minirisc.scomm -> Minirisc.RegisterSet.t
