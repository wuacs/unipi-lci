...
(** A set of Vertexes which ordering is based upon their degree, and if they are
    equal on that value, upon their degree. *)
module VertexSetByDegree = Set.Make (struct
  type t = vertex

  let compare v1 v2 =
    if compare v1.degree v2.degree = 0 then compare v1.id v2.id
    else compare v1.degree v2.degree
end)
...
module VertexSetByColor = Set.Make (struct
  ...
  let compare v1 v2 =
    if compare v1.color v2.color = 0 then compare v1.id v2.id
    else compare v1.color v2.color
end)

(** Vertex set ordered by a metric which takes into account the cost, in terms
    of the numbers of reads and writes of a register, and divides it by its
    degree.

    This metric is a lower is better metric, that is, the best node to spill is
    that which is not used much, so that we push less boilerplate instructions,
    and its remove would remove the most interference, which is what having an
    high degree means. *)
module VertexSetBySpillMetric = Set.Make (struct
  ...
  let compare v1 v2 =
    ...
    if cost1 = cost2 then compare v1.id v2.id else compare cost1 cost2
end)
...
(** Removes a vertex from a vertex set ordered by degree. Such set is used for
    determining the current state of interference, i.e. the interference graph
    passed as input tells us the interference, the vertex set keeps track for
    each node of the remaining nodes in the interference graph. *)
let remove_vertex (interference_graph : RegisterSet.t RegisterMap.t)
    (gi : VertexSetByDegree.t) (regId : register) =
  VertexSetByDegree.filter_map
    (fun v ->
      if v.id = regId then None
      else
        match
          RegisterSet.find_opt regId (RegisterMap.find v.id interference_graph)
        with
        | Some _ ->
...
(** Computes the live ranges of the registers present in the control flow graph
    in input. The registers in the graph are retrieved using
    {! Data_flow_analysis.Utils.get_top} *)
let compute_live_ranges (cfg : mriscfg) =
  let mapState = Data_flow_analysis.liveness_analysis cfg in
  let graph =
    RegisterSet.fold
      (fun reg liveRangeSet ->
  ...
  let add_edge_to_graph from_register to_register graph =
    if to_register = from_register then graph
    else
      RegisterMap.update from_register
        (fun edges ->
          match edges with
          | Some set -> Some (RegisterSet.add to_register set)
          | None -> Some (RegisterSet.singleton to_register))
        graph
  in
  NodeSet.fold
    (fun nodeId liveRangeSet ->
      let live_now = (NodeMap.find nodeId mapState).out_set in
      let res =
        List.fold_right
          (fun instruction (live_now, graph) ->
            let written =
              Data_flow_analysis.Utils.extract_written_register instruction
            in
            match written with
            | Some written ->
                ( RegisterSet.remove written
                    (RegisterSet.union live_now
                       (Data_flow_analysis.Utils.extract_read_registers
                          instruction)),
                  RegisterSet.fold
                    (fun reg graph ->
                      add_edge_to_graph reg written
                        (add_edge_to_graph written reg graph))
                    live_now graph )
            | None -> (live_now, graph))
          (NodeMap.find nodeId cfg.code)
          (live_now, liveRangeSet)
      in
      snd res)
    cfg.nodes graph

(** Given a miniRISC instruction it replaces each reference to register {b x} in
    the instruction passed as first argument with its mapping according to the
    second argument. *)
let replace_register (instruction : scomm) (mapping : register -> register) =
  match instruction with
  | Rtor (op, reg1, reg2, reg3) ->
      Rtor (op, mapping reg1, mapping reg2, mapping reg3)
  | Rtoi (op, reg1, imm, reg2) -> Rtoi (op, mapping reg1, imm, mapping reg2)
  | Rury (op, reg1, reg2) -> Rury (op, mapping reg1, mapping reg2)
  | Load (reg1, reg2) -> Load (mapping reg1, mapping reg2)
  | LoadI (imm, reg1) -> LoadI (imm, mapping reg1)
  | Store (reg1, reg2) -> Store (mapping reg1, mapping reg2)
  | Nop -> Nop

(** This function should be injective, as, together with
    {b unflag_spill_register} need to satisfy the following, for any register
    {b r} with valid positive identifier.

    unflag_spill_register(flag_spilled_register(r)) = r for every r AND the
    register retuned from {b flag_spilled_register} should be identified in such
    a way that {b check_if_spilled_register}, returns true only if the register
    passed as parameter r = flag_spilled_register(x) for some x. *)
...
let rec chaitin_briggs_step1 (interference_graph : RegisterSet.t RegisterMap.t)
    ...
    (gh : VertexSetBySpillMetric.t) (stack : VertexStack.t) (k : int)
    (current_coloring : VertexSetByColor.t) (spilled_registers : RegisterSet.t)
  ...
  | Some v ->
      if v.degree < k then
        ...
        chaitin_briggs_step1 interference_graph
          (RegisterSet.remove v.id registers_left)
          (remove_vertex interference_graph gi v.id)
          gh stack k current_coloring spilled_registers
      else
        chaitin_briggs_step2 interference_graph registers_left gi gh stack k
          current_coloring spilled_registers
  | None ->
      chaitin_briggs_step3 interference_graph registers_left gi gh stack k
        current_coloring spilled_registers

and chaitin_briggs_step2 (interference_graph : RegisterSet.t RegisterMap.t)
    (registers_left : RegisterSet.t) (gi : VertexSetByDegree.t)
    (gh : VertexSetBySpillMetric.t) (stack : VertexStack.t) (k : int)
    (current_coloring : VertexSetByColor.t) (spilled_registers : RegisterSet.t)
    : VertexSetByColor.t * RegisterSet.t =
  let rec find_max_by_metric setmetric =
    ...
    | Some x -> (
        match RegisterSet.mem x.id registers_left with
        | true -> (x, setmetric)
        | false ->
            find_max_by_metric (VertexSetBySpillMetric.remove x setmetric))
    | None ->
        failwith
          "There are no nodes in the graph, thus step2 should have not been \
           invoked"
  in
  let best, gh = find_max_by_metric gh in
  let () = VertexStack.push best stack in
  chaitin_briggs_step1 interference_graph
    (RegisterSet.remove best.id registers_left)
    (remove_vertex interference_graph gi best.id)
    gh stack k current_coloring spilled_registers

and chaitin_briggs_step3 (interference_graph : RegisterSet.t RegisterMap.t)
    (registers_left : RegisterSet.t) (gi : VertexSetByDegree.t)
    (gh : VertexSetBySpillMetric.t) (stack : VertexStack.t) (k : int)
    (current_coloring : VertexSetByColor.t) (spilled_registers : RegisterSet.t)
    : VertexSetByColor.t * RegisterSet.t =
  ...
  | false ->
      ...
      let color, _ =
        VertexSetByColor.fold
          (fun x (acc, b) ->
            match b with
            | false -> (
                match RegisterSet.mem x.id v_set with
                | true ->
                    if x.color < acc then (acc, false)
                    else if x.color = acc then (acc + 1, false)
                    else (acc, true)
                | false -> (acc, b))
            | true -> (acc, b))
          current_coloring (0, false)
      in
      if color < k then
        chaitin_briggs_step3 interference_graph registers_left gi gh stack k
          (VertexSetByColor.add { v with color }
             (VertexSetByColor.remove v current_coloring))
          spilled_registers
      else
        ...
        chaitin_briggs_step1 interference_graph
          (RegisterSet.remove v.id registers_left)
          gi gh stack k
          (VertexSetByColor.add
             { v with color = get_reg_id spilled_id }
             (VertexSetByColor.remove v current_coloring))
          (RegisterSet.add spilled_id spilled_registers)

(** Given a CFG in input, each register that is included in the
    {! spilled_registers} set is considered as a register that needs to be
    spilled. Each implicit read of any such register {b R} in any instruction
    {b X} requires the insertion of the following minirisc instructions:

    + LOADI M Q
    + LOAD Q Q
    + X but with references to R replaced with Q

    where {b Q} is choosen randomly among the free registers in that point in
    the code and {b M} is a memory address which is given randomly to each
    spilled register, starting from 0. In addition, for every operation X' which
    writes to {b R} the following instructions are produced:

    + X' but with references to R replaced by Q'
    + LOADI M Q''
    + STORE Q' Q''

    {b Note:} if instruction {b i} both reads and writes to {b R} then both list
    of instructions are produced and concatenated with the exception of the
    redundant copy of {b i} with references to {b R} updated and Q is reused
    which means Q' = Q.

    {b Warning:} The available registers are considered to be numbered from 0 to
    {b register_number} - 1, if the number of available register is not enough
    to spill registers (because, for example, there are no reserved temporary
    registers or other choices this function fails with) *)
let apply_spilling_to_cfg (cfg : mriscfg) (spilled_registers : RegisterSet.t)
    (register_number : int) =
  let liveness_result = Data_flow_analysis.liveness_analysis cfg in
  let spill_register_local (register : register) (blk : node) (cfg : mriscfg)
      (memory_location : memory_address) : scomm list =
        ...
        let available_registers =
          RegisterSet.diff
            (RegisterSet.remove register
               (RegisterSet.add_seq
                  (Seq.init register_number (fun x -> Id x))
                  RegisterSet.empty))
            (NodeMap.find blk liveness_result).out_set
        in
        fst
          (List.fold_left
             (fun (new_list, available_registers) instruction ->
               let read =
                 Data_flow_analysis.Utils.extract_read_registers instruction
               in
               let written =
                 ...
                 if RegisterSet.mem register read then
                   ...
                   | Some r ->
                       ( [
                           ...
                           Minirisc.Load (r, r);
                           replace_register instruction (fun x ->
                               if x = register then r else x);
                         ]
                         @ new_list,
                         Some r )
                   | None ->
                       raise
                         (NotEnoughRegisters
                            "There are not enough registers to read spilled \
                             register")
                 else (new_list, None)
               in
               let complete_list, available_registers =
                 ...
                 | Some r when r = register -> (
                     ...
                     | Some rr -> (
                         match
                           RegisterSet.choose_opt
                             (RegisterSet.remove rr available_registers)
                         with
                         | Some address ->
                             ( [
                                 Minirisc.LoadI
                                   (get_memory_address memory_location, address);
                                 Minirisc.Store (rr, address);
                               ]
                               @ prefix_list,
                               available_registers )
                         | None ->
                     ...
                     | None ->
                         ...
                         let address_register =
                           match
                             RegisterSet.choose_opt
                               (RegisterSet.remove write_in_register
                                  available_registers)
                           with
                           | Some r -> r
                           | None ->
                         ...
                         ( [
                             replace_register instruction (fun x ->
                                 if x = register then write_in_register else x);
                             Minirisc.LoadI
                               ( get_memory_address memory_location,
                                 address_register );
                           ...
                           available_registers ))
                 | Some r
                   when RegisterSet.mem r read <> true && (check_if_spilled_register r) <> true ->
                     (prefix_list, RegisterSet.add r available_registers)
               ...
               else (complete_list, available_registers))
             ([], available_registers) reversed_instructions)
    | None ->
        raise
          (IllFormedCfg
             "The Cfg passed as input does not contain the needed block of code")
  in
  let _, mapping, blocks =
    RegisterSet.fold
      (fun register (current_address, spilled_mapping, blocks) ->
        ( current_address + 1,
          RegisterMap.add
            (unflag_spill_register register)
            (Address current_address) spilled_mapping,
          NodeMap.fold
            (fun node _ map ->
              NodeMap.add node
                (spill_register_local register node { cfg with code = map }
                   (Address current_address))
                map)
            blocks blocks ))
      spilled_registers
      (0, RegisterMap.empty, cfg.code)
  in
  (blocks, mapping)

(** Returns the CFG in input with each register changed to the register it is
    mapped in {! color_map},

    - If the register is not present in {! color_map} the register is left
      unchanged. *)
let apply_color_map_to_cfg (cfg : mriscfg) (color_map : register RegisterMap.t)
    : mriscfg =
  ...
  let replace_block blk_code =
    List.map
      (Fun.flip replace_register (fun x -> replace_register_with_map x))
      blk_code
  in
  let new_code =
    NodeMap.fold
      (fun blk blk_code acc -> NodeMap.add blk (replace_block blk_code) acc)
      cfg.code NodeMap.empty
  in
  { cfg with code = new_code }

(** Returns the optimized Control Flow Graph, and two memory locations, which
    are, in order, of the input variable and output variable. The memory
    locations are needed since the optimization may have mapped them in
    arbitrary locations, so that one can generate working target code once by
    loading the wanted values in the input memory location and extracting the
    output value by reading from the output location. *)
let chaitin_briggs_algorithm (cfg : mriscfg) (k : int)
    (heuristic : spill_metric) : mriscfg * memory_loc * memory_loc =
  if k < 4 then failwith "Register number must be at least 4"
  else
    let g = compute_live_ranges cfg in
    let vertexesByDegree, vertexesByMetric, vertexesByColor =
      RegisterMap.fold
        (fun reg neighbors (setByDegree, setByMetric, setByColor) ->
          let vertex =
            {
              id = reg;
              degree = RegisterSet.cardinal neighbors;
              cost = heuristic cfg reg;
              color = get_reg_id uncolored_register;
            }
          in
          ( VertexSetByDegree.add vertex setByDegree,
            VertexSetBySpillMetric.add vertex setByMetric,
            VertexSetByColor.add vertex setByColor ))
        g
        ( VertexSetByDegree.empty,
          VertexSetBySpillMetric.empty,
          VertexSetByColor.empty )
    in
    let colors, spilled_registers =
      chaitin_briggs_step1 g
        (Data_flow_analysis.Utils.get_top cfg)
        vertexesByDegree vertexesByMetric (VertexStack.create ()) (k - 2)
        vertexesByColor RegisterSet.empty
    in
    let color_map =
      VertexSetByColor.fold
        (fun v map -> RegisterMap.add v.id (Id v.color) map)
        colors RegisterMap.empty
    in
    let merged_cfg = apply_color_map_to_cfg cfg color_map in
    let spilled_code, spilled_memory_mapping =
      apply_spilling_to_cfg merged_cfg spilled_registers k
    in
    ...
    let output_location =
      ...
      | None -> Register (RegisterMap.find out_register color_map)
    in
    let input_location =
...
let interference_graph_dot (cfg : mriscfg) : string =
  ...
  let other_registers start set =
    RegisterSet.fold
      (fun x acc ->
        acc ^ Printf.sprintf "%d -> %d\n" (get_reg_id start) (get_reg_id x))
      set ""
  in
  let nodes_str =
    RegisterSet.fold
  ...
  let edges_str =
    RegisterMap.fold
      (fun src interf acc -> acc ^ other_registers src interf)
      g ""
  in
...
let translate_cfg_to_target (cfg : mriscfg) (k : int) :
  ...
  let generate_input_instructions input_loc free =
    ...
    | Memory m ->
        [ LoadI (get_memory_address m, free); Store (free, in_register) ]
    | Register r -> [ Rury (Copy, in_register, r) ]
  in
  let generate_output_instructions output_loc free =
    match output_loc with
    | Memory m ->
        [
          LoadI (get_memory_address m, free);
          Load (free, free);
          Rury (Copy, free, out_register);
        ]
  ...
  let rec rec_helper (cfg : mriscfg) (mapping : int Minirisc.LabelMap.t)
    ...
    | Label l -> (
        if Minirisc.LabelMap.mem (Minirisc.Label l) mapping then
          ...
          let blk_mapped_reversed =
            List.filter_map
              (fun x ->
                match remove_garbage x with
  ...
  let code_injected =
    let updated_entry =
      NodeMap.add optimized_cfg.entry
        (generate_input_instructions input_location first_free_register
        @ NodeMap.find optimized_cfg.entry optimized_cfg.code)
        optimized_cfg.code
    in
    NodeMap.add optimized_cfg.exit
      (NodeMap.find optimized_cfg.exit updated_entry
...
let eval_risc_cfg cfg ~registers:k ~value:input =
  ...
  let update_memory memory address value = MemoryMap.add address value memory in
  ...
  let execute_instruction instruction memory registers pc =
    ...
    | Simple scomm -> (
        ...
        | Rury (op, r1, r2) ->
            let v1 = fetch_register registers r1 in
            let result =
              match op with Not -> if v1 = 0 then 1 else 0 | Copy -> v1
            in
            (memory, update_register result r2 registers, pc + 1)
        | Load (r1, r2) ->
        ...
        | LoadI (imm, r1) -> (memory, update_register imm r1 registers, pc + 1)
        | Store (r1, r2) ->
  ...
  Fun.protect
    ...
    (fun _ ->
      ...
      | Some imp_prog ->
          ...
          then failwith "Undefined variable found"
          else
            let out_channel = open_out target_file_path in
            Fun.protect
              ~finally:(fun _ -> close_out out_channel)
              (fun _ ->
                let oc = open_out target_file_path in
                Printf.fprintf oc "%s"
                  (generate_target_code_string risc_cfg register_number))
      | None ->
          failwith
            (Printf.sprintf "Error while parsing file %s\n" miniimp_file_path))

let compile_and_run_imp_from_file ?(register_number = 4) miniimp_file_path
    ~input ~check_undefinedness : int =
  if register_number < 4 then failwith "Register number must be >= 4";
  let in_channel = open_in miniimp_file_path in
  Fun.protect
    ~finally:(fun _ -> close_in in_channel)
    (fun _ ->
      ...
      | Some imp_prog ->
          ...
          if
            ...
            && Data_flow_analysis.check_for_undefinedness risc_cfg
          then failwith "Undefined variable found"
          else eval_risc_cfg risc_cfg ~registers:register_number ~value:input
      | None ->
          failwith
            (Printf.sprintf "Error while parsing file %s\n" miniimp_file_path))
