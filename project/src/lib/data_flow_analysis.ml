...
(** Computes the live analysis and a string in the Dot language in which each
    node contains the live_in and live_out sets *)
let live_analysis_dot (cfg : Minirisc.scomm Cfg.control_flow_graph) : string =
  let pretty_print_live_sets (bas : Data_flow_utils.block_analysis_state) :
      string =
    "Live-IN: "
    ^ Minirisc.RegisterSet.fold
        (fun reg acc ->
          acc ^ "R" ^ string_of_int (Minirisc.get_reg_id reg) ^ "\\n")
        bas.in_set "[ "
    ^ " ]\n    \\n Live-OUT: "
    ^ Minirisc.RegisterSet.fold
        (fun reg acc ->
    ...
    ^ " ]"
  in
  let liveness_result = Liveness_analysis.liveness_analysis cfg in
  let nodes_str =
    Cfg.NodeSet.fold
      (fun node acc ->
        acc
...
let defined_analysis_dot (cfg : Minirisc.scomm Cfg.control_flow_graph) : string
    =
  let pretty_print_defined_sets (bas : Data_flow_utils.block_analysis_state) :
      string =
    "Defined-IN: "
    ^ Minirisc.RegisterSet.fold
        (fun reg acc ->
    ...
    ^ " ]"
  in
  let defined_result = Defined_analysis.defined_analysis cfg in
  let nodes_str =
    Cfg.NodeSet.fold
      (fun node acc ->
        acc
...
let check_for_undefinedness = Defined_analysis.check_for_undefinedness
let liveness_analysis = Liveness_analysis.liveness_analysis
