...
let fibonacci n =
  let rec helper v1 v2 n = if n <= 1 then v2 else helper v2 (v1 + v2) (n - 1) in
  helper 1 0 n

let factorial n =
  let rec help n t = if n < 2 then t else help (n - 1) (n * t) in
  help n 1

let sum_first_naturals n = n * (n + 1) / 2

let power2 n =
  let rec helper t v = if t < 1 then v else helper (t - 1) v * 2 in
  helper n 1
  ...
  List.iter
    (fun x ->
      Alcotest.check Alcotest.int
        ...
        (oracle x)
        (Target_code.compile_and_run_imp_from_file ~register_number:4
           (input_dir ^ filename) ~input:x ~check_undefinedness:true))
    inputs
...
let test_sumnaturals () =
  compute_and_test ~filename:"sumnaturals.miniimp" ~oracle:sum_first_naturals
    ~tcs:100
...
let () =
  run "Target code tests"
    [
      ( "Target Code From File",
        [
