...
let test_defined_analysis () =
  ...
  let output_dir =
    Sys.getcwd () ^ "/../../../test/defined_analysis/expected/"
  in
  ...
  |> List.iter (fun input_file ->
         ...
         try 
          let expected_channel =
            open_in (output_dir ^ Filename.chop_extension input_file ^ ".out")
          in
          let actual_output = Lexing.from_channel input_channel in
          match Miniimp.parse_with_errors actual_output with
          | None ->
              Alcotest.fail
                "Error in parsing MiniImp in Defined Analysis testing"
          | Some prog -> (
              ...
              with End_of_file ->
                ...
                Alcotest.fail
                  ("File " ^ input_file
                  ^ " does not contain a boolean for defined analysis"))
              with Sys_error _ -> Alcotest.check Alcotest.bool (Printf.sprintf "No file of output for program %s" input_file) true true) 

let () =
  run "Data Flow Tests"
    [
      ( "Defined Analysis Tests",
        [ test_case "test_defined_analysis" `Quick test_defined_analysis ] );
    ]
