...
let string_of_value value = 
  ...
  | Closure(x, _, _) -> String.cat "A non recursive closure with input parameter " x
  | _ -> "idk"
...
in
  let rec rec_eval ast env = 
    ...
    | App(f, par) -> (match (rec_eval f env) with 
                    | (Closure(v, a, e)) -> rec_eval a (updateEnv v (rec_eval par env) e)
                    | (RecClosure(f, v, a, e)) -> rec_eval a (updateEnv f (RecClosure(f, v, a, e)) (updateEnv v (rec_eval par env) e))
    ...
    | LetFun(f, p, a1, a2) -> rec_eval a2 (updateEnv f (RecClosure(f, p, a1, env)) env)
