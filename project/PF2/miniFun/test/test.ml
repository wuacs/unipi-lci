...
(* A simple test case *)
let test_eval0 _ =
  let result = eval (App(Value(Closure("x", Op(Var("x"), Plus, Value(Integer(4))), EnvMap.empty)),
                         Value(Integer(5)))) EnvMap.empty in
  match result with
...
let test_eval1 _ =
  try let _ = (eval (App(Value(Closure("x", Op(Var("x"), Plus, Value(Integer(4))), EnvMap.empty)),
                         Value(Boolean(true)))) EnvMap.empty)
  in assert_failure "Exception taken care"
  with
...
let test_eval2 _ = 
  let internal = If(Op(Var("x"), Equal, Value(Integer(0))), Value(Integer(1)), Op(Var("x"), Mul, App(Var("factorial"), (Op(Var("x"), Minus, Value(Integer(1)))))))
  in
  let program = LetFun("factorial", "x", internal, (App(Var("factorial"), Value(Integer(5)))))
  in
